...
public class PasswordEncodingTest {
    ...
    void testLdap() {
        ...
        String encodedPwd2 = ldap.encode(PASSWORD);
        System.out.println(encodedPwd);
        System.out.println(encodedPwd2);

        assertTrue(ldap.matches(PASSWORD, encodedPwd));
        assertTrue(ldap.matches(PASSWORD, encodedPwd2));
    }

    @Test
    void testNoOp() {
        PasswordEncoder noOp = NoOpPasswordEncoder.getInstance();
        System.out.println(noOp.encode(PASSWORD)); // NoOpPasswordEncoder doesn't change it, so it returns the same string "password"
    }
